...
Linear::Linear(size_t in_features, size_t out_features, bool bias, std::string name) {
    weight_.tensor = Tensor(0.f, {out_features, in_features});
    initialize_tensor_normal(weight_.tensor, 0.f, std::sqrt(1.f/in_features));
    register_parameter(std::make_shared<Parameter>(weight_));

    if (bias) {
        bias_.tensor = Tensor(0.f, {out_features});
        register_parameter(std::make_shared<Parameter>(bias_));
    }
