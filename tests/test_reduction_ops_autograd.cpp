...
TEST_CASE("Tensor mean autograd") {
    ...
    t1.zero_grad();
    res2.backward();
    CHECK(check_tensor_equal(t1.grad(), Tensor({1.0/2,1.0/2,1.0/2,1.0/2,1.0/2,1.0/2}, {2,3})));

    Tensor res3 = mean(t1, 1);
    t1.zero_grad();
    res3.backward();
    CHECK(check_tensor_equal(t1.grad(), Tensor({1.0/3,1.0/3,1.0/3,1.0/3,1.0/3,1.0/3}, {2,3})));
}
TEST_CASE("Tensor max autograd") {
    ...
    t1.zero_grad();
    res2.backward();
    CHECK(check_tensor_equal(t1.grad(), Tensor({0,0,0,1,1,1}, {2,3})));

    Tensor res3 = max(t1, 1);
    t1.zero_grad();
    res3.backward();
    CHECK(check_tensor_equal(t1.grad(), Tensor({0,0,1,0,0,1}, {2,3})));
...
TEST_CASE("Tensor min autograd") {
    ...
    t1.zero_grad();
    res2.backward();
    CHECK(check_tensor_equal(t1.grad(), Tensor({1,1,1,0,0,0}, {2,3})));

    Tensor res3 = min(t1, 1);
    t1.zero_grad();
    res3.backward();
    CHECK(check_tensor_equal(t1.grad(), Tensor({1,0,0,1,0,0}, {2,3})));
